    ...
    <div class="lg:w/8/12 flex-1 flex flex-col lg:items-start items-center">
      ...
      <div class="capitalize flex font-semibold space-x-3 text-center text-sm my-2">
        ...
        <div>
          ...
          <div
            class="bg-white w-56 shadow-md mx-auto p-2 mt-12 rounded-md text-gray-500 hidden text-base dark:bg-gray-900 uk-drop"
            uk-drop="mode: click"
          >
            <ul class="space-y-1">
              <li>
      ...
      <div class="divide-gray-300 divide-transparent divide-x grid grid-cols-3 lg:text-left lg:text-lg mt-3 text-center w-full dark:text-gray-100">
        ...
        <div class="lg:pl-4 flex lg:flex-row flex-col">
          {{ profile.followers_count }} <strong class="lg:pl-2">Följare</strong>
        </div>
        <div class="lg:pl-4 flex lg:flex-row flex-col">
          {{ profile.follows_count }} <strong class="lg:pl-2">Följer</strong>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  async fetch () {
    ...
    this.profile = await fetch(
      `${this.$config.facebookBaseApi}/${this.$config.facebookVersionApi}/${igid}?fields=${fields}&access_token=${this.$config.facebookTokenApi}`,
      {
        method: 'GET',
        headers: {},
      }
